...
list_of_no_inputs = [5]#,1000, 1200, 1500, 2000]
list_of_no_gates = [100000]#,200000, 400000, 700000, 1000000]
list_of_no_levels = [150]#,200, 290, 300, 400]
clock_input = "BM_CLK"
reset_input = "BM_RST"
...
def CombinationRepetitionUtil(chosen, arr, index,
    ...
    if index == r:
        instant_combination=[]
        for j in range(r):
            instant_combination.append(chosen[j])
            # print(chosen[j], end = " ")
             
        combination_r.append(instant_combination)
        return
...
def CombinationRepetition(arr, n, r):
    ...
    for ele in r:
        ...
        CombinationRepetitionUtil(chosen, arr, 0, ele, 0, n)
...
def writing_dff_gates_func(file_stream_fp,dictionary_inst,graph_dict_num_inst,dff_node_name):
    ...
    file_stream_fp.write(" GATE_NODE_N")
...
for i in range(len(list_of_no_gates)):
    ...
    primary_inputs_fan_out_degree_max = -1
    ...
    if 1:
        with open(output_path,'w')as fp_rule_file:
            ...
            for i in range(no_of_graphs):
                ...
                for node in inst_graph_obj.nodes():
                    if inst_graph_obj.in_degree(node) == 0:
                        if inst_graph_obj.out_degree(node) > primary_inputs_fan_out_degree_max and node not in dict_list[i+1]["CLOCK"] and node not in dict_list[i+1]["RESET"]:
                            primary_inputs_fan_out_degree_max = inst_graph_obj.out_degree(node)                            
                    
            #####################################################################################################################################
#            print(primary_inputs_fan_out_degree_max)
#            sys.exit("combination")
            ...
            input_combo_arr = []
            input_placing = []
            for input_num in range(max_inputs-1):
                input_combo_arr.append(input_num+1)
            for input_placing_combo in range(primary_inputs_fan_out_degree_max):
                input_placing.append(input_placing_combo+1)
            input_combo_arr_len = len(input_combo_arr) - 1

            CombinationRepetition(input_combo_arr, input_combo_arr_len, input_placing)
#            print(combination_r)
#            sys.exit("combination")
            ...
            #########################################PRIMARY INPUT connected IC_REMOVAL_rule#################################################
            for input_combo_index, input_combo_inst in enumerate(combination_r):
                fp_rule_file.write("\n\n\trule primary_input_connected_IC_connection_removal_"+str(input_combo_index+1)+" {")

                fp_rule_file.write("\n\t\tsub {")
                fp_rule_file.write("\n\t\t\tU1 INPUT_G[connected = \"1\" ];")

                fp_rule_file.write("\n\t\t\tU2 IC_INSTANCE [ connected = \"1\"]")
                fp_rule_file.write("(U1.OUT1 -> IN1)")
                fp_rule_file.write(";")

                for inp_combo_ele_index,inp_combo_ele in enumerate(input_combo_inst):
                    fp_rule_file.write("\n\t\t\tU3_"+str(inp_combo_ele_index)+" GATE_NODE_N[ connected = \"0\"](")
                    fp_rule_file.write("U2.OUT1 -> IN"+str(inp_combo_ele))
                    fp_rule_file.write(")")
                    fp_rule_file.write(";")
                ...
                for inp_combo_ele_index,inp_combo_ele in enumerate(input_combo_inst):
                    fp_rule_file.write("\n\t\tdel U3_"+str(inp_combo_ele_index)+" GATE_NODE_N(")
                    fp_rule_file.write("U2.OUT1 -> IN"+str(inp_combo_ele))
                    fp_rule_file.write(")")
                    fp_rule_file.write(";")

                    fp_rule_file.write("\n\t\tadd U3_"+str(inp_combo_ele_index)+" (")
                    fp_rule_file.write("U1.OUT1 -> IN"+str(inp_combo_ele))
                    fp_rule_file.write(")")
                    fp_rule_file.write(";")
            ...
            #########################################PRIMARY INPUT connected IC_REMOVAL_rule#################################################
#            fp_rule_file.write("\n\n\trule primary_input_connected_IC_instance_removal {")
#
#            fp_rule_file.write("\n\t\tsub {")
#            fp_rule_file.write("\n\t\t\tU1 INPUT_G[connected = \"1\" ];")
#
...
#            fp_rule_file.write("\n\t}")
            ...
            #########################################NT connected IC_REMOVAL_rule#################################################
            for input_combo_index, input_combo_inst in enumerate(combination_r):
                fp_rule_file.write("\n\n\trule nt_connected_IC__connection_removal_"+str(input_combo_index+1)+" {")
                ...
                fp_rule_file.write("del U2 IC_INSTANCE [ connected = \"1\"](U1.OUT1 -> IN1);");
...
#            fp_rule_file.write("\n\t}")
            ...
            if 1:
                ...
                for input_combo_index, input_combo_inst in enumerate(combination_r):
                    fp_rule_file.write("\n\t\t\tprimary_input_connected_IC_connection_removal_"+str(input_combo_index+1)+" : *;")

#                fp_rule_file.write("\n\t\t\tprimary_input_connected_IC_instance_removal : *;")
                for input_combo_index, input_combo_inst in enumerate(combination_r):
                    fp_rule_file.write("\n\t\t\tnt_connected_IC__connection_removal_"+str(input_combo_index+1)+" : *;")
