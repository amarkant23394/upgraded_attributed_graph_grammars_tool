...
def writing_dff_gates_func(file_stream_fp,dictionary_inst,graph_dict_num_inst,dff_node_name,primary_input_sub_rule=1):
    ...
    for input_connected_node in dff_inputs_connected_need_2_shifted:
        if dff_input_connected_num > 1:
            file_stream_fp.write(", ")
        if primary_input_sub_rule == 0:
            file_stream_fp.write("IN_"+str(input_connected_node)+".OUT1 -> "+"IN"+str(dff_input_connected_num))
        else:
            file_stream_fp.write("GATE_"+str(input_connected_node)+".OUT1 -> "+"IN"+str(dff_input_connected_num))
        dff_input_connected_num += 1     
...
#            sys.exit("combination")
            ...
            if 1:
                ...
                for primary_input_rule in range(2):
                    for graph_num, graph_dict_inst in enumerate(list_of_dictionaries):
                        ...
                        if primary_input_rule == 0:
                            fp_rule_file.write("\n\n\trule primary_input_connected_pattern_substitute_"+str(graph_num)+" {")
                        else:
                            ...
                            if str(graph_node_attributes[node_name]) != "DFF":
                                ...
                                for input_connected_node in inputs_connected_need_2_shifted:
                                    ...
                                    if input_connected_node in clk_list and graph_node_attributes[input_connected_node] == "INPUT":
                                        ...
                                        if primary_input_rule == 0:
                                            fp_rule_file.write("IN_"+str(input_connected_node)+".OUT1 -> "+"IN"+str(input_connected_num))
                                        else:
                                            fp_rule_file.write("GATE_"+str(input_connected_node)+".OUT1 -> "+"IN"+str(input_connected_num))

                                    input_connected_num += 1               

                                fp_rule_file.write(");")
                            else:
                                writing_dff_gates_func(fp_rule_file,list_of_dictionaries[graph_num],graph_num,node_name,primary_input_rule)
            ...
            if 1:
                ...
                ############Calculation for the repeating patterns########
                        ...
                        rules_no_to_remove_ic.append(curr_level_graphs_index.copy())
                ...
                for levels_index in range(len(rules_no_to_remove_ic)):
                    for graph_num, graph_dict_inst in enumerate(list_of_dictionaries):
                        if levels_index == 0:
                            fp_rule_file.write("\n\t\t\tprimary_input_connected_pattern_substitute_"+str(graph_num)+" : *;");
                        else:
